...
def draw_logo(input_path, logo_files, places, output_path):
    ...
    for place, size in places:
        ...
        logo_file = logo_files.pop(index).resize((size, size), Image.BICUBIC)
        image.paste(logo_file, place, logo_file)
    image.save(output_path, "PNG")


def shirt():
    logo_files = list(map(lambda logo: Image.open("logo/"+logo).convert('RGBA'), techs))
    draw_logo("empty_frontback.png", logo_files, front_places, "front.png")
    draw_logo("empty_frontback.png", logo_files, back_places, "back.png")
    draw_logo("empty_side.png", logo_files, left_places, "left.png")
    draw_logo("empty_side.png", logo_files, right_places, "right.png")
